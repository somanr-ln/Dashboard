// Get JSON data
function drawTreeLayout(divId, chartData) {
	jq.getScript('js/lib/d3.v3.min.js', function() {
		var treeData = jq.parseJSON(chartData);
	    ...
	    dragListener = d3.behavior.drag()
	        ...
	        .on("drag", function(d) {
	            ...
	            relCoords = d3.mouse(jq('svg').get(0));
	            if (relCoords[0] < panBoundary) {
	                panTimer = true;
	                pan(this, 'left');
	            } else if (relCoords[0] > (jq('svg').width() - panBoundary)) {
	                ...
	                panTimer = true;
	                pan(this, 'up');
	            } else if (relCoords[1] > (jq('svg').height() - panBoundary)) {
	                panTimer = true;
	                pan(this, 'down');
	            } else {
	                try {
